 ...
 contains
   function gravity_disturbance(h, latitude, longitude, GM, R, GM_ellipsoid, a, b, f_reciprocal, omega, C_bar_nm, nmax, p_bar_nm, dp_bar_nm) result(delta_g_h)
    ...
    !!
      ...
      !delta_g_h = sqrt((dW_dr_temp + omega**2*rr*cos(Radian(latitude))**2)**2 + (1.0_longdp/rr * (dW_dtheta_temp - omega**2*rr**2*cos(Radian(latitude))*sin(Radian(latitude))))**2 + (1.0_longdp/(rr*cos(Radian(latitude))) * dW_dlambda_temp)**2) - sq
